...
export async function POST(request: Request) {
  try {
    ...
    if (paymentMethod !== "COD") return NextResponse.json({ error: "Invalid payment method." }, { status: 400 });

    for (const item of items) {
      if (typeof item.id !== "string" || typeof item.variantId !== "string" || !item.variantId || !Number.isInteger(item.quantity) || item.quantity <= 0) {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { name: true, email: true, customerType: true, founderDiscount: true },
    });
    ...
    const referredFounder = referralCode
      ? await prisma.user.findFirst({ where: { founderCode: referralCode, customerType: "FOUNDER" }, select: { id: true, founderCode: true, affiliateCommission: true } })
      : null;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const createdOrder = await tx.order.create({
        data: {
          userId: session.user.id, status: "PENDING", paymentStatus: "PENDING",
          subtotal: subtotal.toFixed(2), shippingCost: shippingCost.toFixed(2), discount: discount.toFixed(2), total: total.toFixed(2),
          customerName, customerEmail, shippingAddress, shippingCity, shippingPostalCode: shippingPostalCode || null, paymentMethod,
